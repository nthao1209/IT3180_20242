...
//   filename: string;
//   state: 'pending' | 'complete' | 'error';
//   fileId?: string; // Lưu trữ fileId từ Google Drive
// };

// interface InputProps {
//   onFilesAdded?: (addedFiles: { url: string; id: string }[]) => Promise<void>;
//   onFileDelete?: (url: string, id: string) => void;
//   photos?: { url: string; id?: string }[]; // Mảng các object chứa url và id (nếu có)
// }
...
//       for (const uploadInfo of addedFiles) {
//         if (uploadInfo.file) {
...
//               ...uploadInfo,
//               downloadUrl: `https://drive.google.com/uc?export=view&id=${data.fileId}`, // Tạo public URL ở client
//               state: 'complete',
//               fileId: data.fileId,
//             }));
//           } else {
//             promises.push(Promise.resolve({ ...uploadInfo, state: 'error' }));
//             const errorData = await response.json();
//             console.error('Lỗi tải lên:', errorData?.error);
//             setError(errorData?.error || 'Lỗi tải lên không xác định');
//           }
//         }
//       }
...
//           const result = results.find(r => r?.filename === state.filename);
...
//         .filter(r => r?.downloadUrl && r?.fileId)
//         .map(r => ({ url: r!.downloadUrl, id: r!.fileId! }));

//       if (uploadedFilesData.length > 0) {
//         await onFilesAdded?.(uploadedFilesData);
//       }

//     } catch (error) {
//       console.error('Lỗi chung khi tải lên:', error);
//       setError('Lỗi chung khi tải lên');
//     } finally {
//       setUploading(false);
//     }
//   }, [onFilesAdded]);
...
//   const _handleDelete = async (photourl: string, fileId?: string) => {
//     if (fileId) {
//       console.log("before delete: ",imageStates);
...
//       if (response.ok) {
//         console.log("after delete: ",imageStates);
//         const newState = imageStates.filter(state => state.downloadUrl !== photourl);
//         setImageStates(newState);
//         console.log("new state: ",newState);
//         console.log("after update: ",imageStates);
//         onFileDelete?.(photourl, fileId);
//       } else {
//         const errorData = await response.json();
//         console.error('Lỗi xóa:', errorData?.error);
//         setError(errorData?.error || 'Lỗi xóa không xác định');
//       }
//     } else {
//       // Nếu không có fileId (ảnh đã tồn tại trước khi tích hợp Google Drive),
//       // bạn có thể có logic xóa khác hoặc bỏ qua.
//       const newState = imageStates.filter(state => state.downloadUrl !== photourl);
//       setImageStates(newState);
//       onFileDelete?.(photourl, ''); // Truyền ID rỗng hoặc xử lý khác
//     }
//   };
...
//         {imageStates.map((image, index) => (
//           <div key={index} className='border-0 p-0 w-full relative shadow-md bg-slate-200 rounded aspect-square h-full'>
//             {image.state === 'complete' ? (
//               <Image
//                 fill
//                 src={image.downloadUrl}
...
//               onClick={(e) => {
//                 e.stopPropagation();
//                 _handleDelete(image.downloadUrl, image.fileId);
//               }}
//             >
//               <div className="flex h-4 w-4 -translate-x-2 translate-y-2 cursor-pointer items-center justify-center">
//                 <X width={16} height={16} className='bg-red-500 text-white rounded-lg' />
