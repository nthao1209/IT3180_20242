...
async function OnHold() {
  ...
  const getRankForUser = async (book_id: number) => {
    const rank = await prisma.$queryRawTyped(
      getReservationRankForBook(book_id, session?.user.user_id as number)
    );
  ...
  return (
    <div className="space-y-2">
      {results.map((result) => (
        <div
          key={result.book_id}
          className="flex flex-col sm:flex-row p-4 sm:space-x-4 border rounded-sm justify-between"
        >
          ...
          <div className="flex flex-col space-y-1 max-w-2xl">
            <div className=" text-green-600 bg-green-50 p-2 border-l-4 border-green-500 ">
              <p className="font-bold">
                You are # {getRankForUser(result.book_id)} in the waitlist.
              </p>
            </div>
