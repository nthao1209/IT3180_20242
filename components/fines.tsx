    ...
    <div>
      {results.length > 0 ? (
        results.map((result) => (
          <div
            key={result.fine_id}
            className="flex flex-col lg:flex-row p-4 sm:space-x-4 border rounded-sm justify-evenly"
          >
            <div className="flex-grow max-w-md space-y-1 mb-1">
              ...
              <div>
                <p>
                  Checkout date:{" "}
                  {format(
                    getDateWithOffset(result.borrowings.borrow_date),
                    "MMM dd, yyyy"
                  )}
                </p>
                <p>
                  Return date:{" "}
                  {format(
                    getDateWithOffset(result.borrowings.return_date!),
                    "MMM dd, yyyy"
                  )}
                </p>
              </div>
            ...
            <div className="flex flex-col space-y-2 border rounded-sm p-2 bg-slate-50 justify-between">
              <div className=" text-red-600 bg-red-50 p-2 border-l-4 border-red-500 ">
                <p className="font-bold">
                  Fine:{" "}
                  {formatAmountForDisplay(
                    result.fine_amount as unknown as number,
                    "CAD"
                  )}
                </p>
              </div>
