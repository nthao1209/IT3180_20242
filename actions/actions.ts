...
export async function requestDeleteBook(book_id: number, path: string) {
  ...
  // Kiểm tra xem người dùng có phải là tác giả của sách không
  const book = await prisma.books.findUnique({
    where: { book_id },
    select: { author_id: true },
  });
  ...
  // Kiểm tra xem đã có yêu cầu xóa đang chờ duyệt cho sách này chưa
  const existingRequest = await prisma.book_requests.findFirst({
    where: {
      book_id,
      action: "delete",
...
// //             fine?.fine_amount as unknown as number,
// //             "CAD"
// //           ),
// //         },
// //       },
