...
export async function updateUser(user_id: number, username: string, name: string, email: string, date_of_birth: string, gender: string, role: string, path: string) {
    ...
    const dobDate = new Date(date_of_birth);
    console.log(date_of_birth, "Ngày ")
    console.log("Vai trò ", role)
    console.log("Ngày sinh:", dobDate)

    if (isNaN(dobDate.getTime())) {
        console.error("Lỗi: Ngày sinh không hợp lệ. Vui lòng kiểm tra định dạng hoặc giá trị.");
        throw new Error("Ngày sinh không hợp lệ");
    }
    const genderEnum = Gender[gender as keyof typeof Gender];
...
export async function approveBookRequest(requestId: number, path: string) {
  ...
  await prisma.$transaction(async (tx) => {
    const { action, book_id, details, author_id: requesterId } = req

    // 2. Parse JSON trong details
    let parsedDetails: {
      name?: string
      isbn?: string
      category?: number[]
    ...
    try {
      ...
      throw new Error('Invalid request details format')
    }
    ...
    if (action === 'add') {
      ...
      // 3b. Tạo các liên kết category chỉ với ID tồn tại
      if (Array.isArray(parsedDetails.category) && parsedDetails.category.length > 0) {
        // Lấy danh sách category_id hợp lệ
        const existingCats = await tx.book_categories.findMany({
          where: {
            category_id: { in: parsedDetails.category },
          },
          select: { category_id: true },
        })
        const validCategoryIds = existingCats.map((c) => c.category_id)

        for (const categoryId of validCategoryIds) {
          await tx.book_category_links.create({
            data: {
        ...
        // Các category_id không tồn tại sẽ bị bỏ qua
      }
      ...
      // 4b. Cập nhật category: xóa cũ, tạo mới với ID tồn tại
      if (Array.isArray(parsedDetails.category)) {
        // Xóa các book_category_links hiện có
        await tx.book_category_links.deleteMany({
          where: { book_id },
        })

        // Lấy lại category_id hợp lệ
        const existingCats = await tx.book_categories.findMany({
          where: {
            category_id: { in: parsedDetails.category },
          },
...
export async function rejectBookRequest(requestId: number, path: string) {
  const exists = await prisma.book_requests.findUnique({
    where: { request_id: requestId },
  })
...
////////////////////////////////////////////////////////////////////////////////
// Các hàm like/unlike giữ nguyên
////////////////////////////////////////////////////////////////////////////////
